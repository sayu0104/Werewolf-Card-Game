...
@Table(name = "users") // この世界（java）では「User」と呼ぶが、データベースの世界では「users」というテーブル名で保存すること（DBでは小文字＆複数形の users
						// にする（設計書の命名ルールに合わせるため））
public class User {// このクラス（User）を他のパッケージからでもアクセスできるように公開する
	...
	@Column(name = "is_admin", nullable = false) // （名前は"is_admin"、空欄不可）
	private Boolean isAdmin = false;// 管理者かどうか（true/false）。isは「真偽値」の目印で機能はなく、ただの名前
	...
	public Boolean getIsAdmin() {
		return isAdmin;// 管理者かどうかの値を返す（読む窓口）
	}

	public void setIsAdmin(Boolean isAdmin) {
		this.isAdmin = isAdmin;// 管理者かどうかを書き換える（書く窓口）
	}
